  ...
  return (
    <>
      <div className="min-h-screen">
        {/* header */}
        <div className="h-14 bg-gray-600 flex flex-flow p-2">
          <div className="basis-1/3"></div>
          <div className="basis-2/3 flex flex-flow justify-end pr-2 text-blue-500">
            {!isConnected ? (
              <Button variant="contained" startIcon={<Wallet />} onClick={() => connect({ connector: injected() })}>
                链接钱包
              </Button>
              ...
              <>
                {isLoading ? (
                  ...
                  <span className="mr-2 h-10 text-base/10">
                    余额: {balance ? Number(formatEther(balance.value)).toFixed(3) : '--'} {balance?.symbol}
                  </span>
                )}
                {
                isLinking ? <Button loading loadingPosition="start" startIcon={<Wallet />} variant="outlined">{shortenAddress(address as string)}</Button> :
