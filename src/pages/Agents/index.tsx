...
import type { Agent } from '@apis/model/Agents';
...
const Agents = () => {
  ...
  useEffect(() => {
    if (agentRes && agentRes.data && agentRes.totalPages) {
  ...
  const AgentCard = (agent: Agent) => {
    ...
    return (
      <div
        ...
        className="bg-gray-800/60 border-gray-600/40 w-74 border-2 rounded-2xl p-2 transition-all duration-300 hover:shadow-lg hover:scale-105 hover:bg-gray-700/60 backdrop-blur-sm"
      >
        {/* 头部信息 */}
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-base text-white">{agent.agentName}</h3>
              {/* 评分 */}
        ...
        {/* 描述 */}
        <p className="text-gray-300 text-sm mb-3 line-clamp-2">{agent.description}</p>
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 bg-gradient-to-br min-h-screen text-white">
      {/* 搜索框 */}
      ...
      <div className="flex justify-between items-center mt-8 mb-8">
        <h1 className="text-2xl font-bold text-white">所有代理</h1>
        <span className="text-gray-300">{agents.length} 个结果</span>
      </div>
      ...
      <div className="flex items-center justify-center gap-2">
        <button
          ...
          className="flex items-center gap-1 px-3 py-2 rounded-lg text-sm font-medium text-gray-300 hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
        ...
        {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
          <button
            ...
            className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
              currentPage === page ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-gray-700'
            }`}
          >
            {page}
          </button>
